...
const ShareScreen = ({ navigation, route }) => {
    const { mergedImage: initialImage, photoId } = route.params || {};
    const [mergedImage, setMergedImage] = useState(initialImage);
                        ...
                        {shareOptions.map((option, index) => (
                            <TouchableOpacity key={index} style={styles.socialItem}>
                                <View style={styles.socialIconBg}>
                                    <CommunityIcon name={option.icon} size={30} color={option.color} />
                                </View>
