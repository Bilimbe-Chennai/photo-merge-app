...
const CameraView = forwardRef(({ template, onReady, cameraPosition = 'front' }, ref) => {
  ...
  const format = React.useMemo(() => {
    ...
    console.log(`[Camera] Selected Format: ${bestFormat?.photoWidth}x${bestFormat?.photoHeight} HDR:${bestFormat?.supportsPhotoHdr} Vid:${bestFormat?.videoWidth}x${bestFormat?.videoHeight}`);
    return bestFormat;
  }, [device]);

  if (!hasPermission) return <View style={StyleSheet.absoluteFill}><Text style={{ color: 'white', textAlign: 'center', marginTop: 50 }}>No Camera Permission</Text></View>;
  if (!device) return <View style={StyleSheet.absoluteFill}><Text style={{ color: 'white', textAlign: 'center', marginTop: 50 }}>No Camera Device</Text></View>;
